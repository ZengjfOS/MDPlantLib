...
function refreshReadmeDocsTable(outputFile, subProjectDocsDir) {
    ...
    logger.info("refresh readme: " + subProjectDocsDir)
...
function generateIndexTable(rootPath, relativePath, suffix) {
    ...
    logger.info("generate index table: " + rootPath + "/" + relativePath)
...
function isTable(textBlock, rootPath, cursorOffset) {
    ...
    logger.info("enter isTable")

    if (textBlock[cursorOffset].trim().length == 0)
        return {"status": false, "error": false, "info": info}
    ...
    if (textBlock[startLine].trim().replace(/\s*/gi, "") == "NO.|文件名称|摘要") {
        logger.info("table for docs found")
        found = true
        info = "docs"
    }else if (textBlock[startLine].trim().replace(/\s*/gi, "") == "NO.|文件名称") {
        logger.info("table for index found")
        found = true
        info = "index"
    ...
    return {"status": found, "error": false, "info": info}
}
