 ...
 * @param {function} cb 
 */
function getHTTPImage(umlString, plantumlServer, type, outputPath, cb) {
    ...
    http.get(plantumlServer + '/' + type +  '/' + umlEncoded, (res) =>{
        let datas = []
        let size = 0
    ...
    axios({
        method: "get",
        url: plantumlServer + '/' + type +  '/' + umlEncoded,
        responseType: "stream"
 ...
 * @returns 
 */
function getHTTPSVGImage(umlString, plantumlServer, outputPath, cb) {
    return getHTTPImage(umlString, plantumlServer, "svg", outputPath, cb)
}
 ...
 * @returns 
 */
function isPlantuml(textBlock, rootPath, cursor) {
    let found = false
