...
function convertRowColume2Table(lineValue) {
    tableRegex = new RegExp("\\s*table[\\s:]*(\\d*)[x\\s*]*(\\d*)\\s*")
    outputString = ""
...
function convert2Table(lineValue) {
    tableRowColumeRegex = new RegExp("\\s*table[\\s:]*(\\d*)[x\\s*]*(\\d*)\\s*", "g")
    tableJSONRegex = new RegExp("\\s*table[\\s:]*([\\w\\/]*\\.json)", "g")

    tableMatchValue = tableRowColumeRegex.exec(lineValue)
